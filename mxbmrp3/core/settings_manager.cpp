...
namespace {
    ...
    namespace Keys {
        ...
        namespace Records {
            ...
            constexpr const char* AUTO_FETCH = "autoFetch";
        }
    ...
    void saveRecordsColumns(SettingsManager::HudSettings& settings, uint32_t cols) {
        ...
        // Sectors saved as combined flag (all 3 on/off together)
    ...
    void loadRecordsColumns(const SettingsManager::HudSettings& settings, uint32_t& cols) {
        ...
        // Sectors loaded as combined flag (all 3 on/off together)
...
void SettingsManager::captureToProfile(const HudManager& hudManager, ProfileType profile) {
    ...
    // Capture RecordsHud
    {
        ...
        settings["autoFetch"] = hud.m_bAutoFetch ? "1" : "0";
    ...
    // LeanWidget with enabledRows and showMaxMarkers
    {
        HudSettings settings;
...
void SettingsManager::applyProfile(HudManager& hudManager, ProfileType profile) {
    ...
    // Apply RecordsHud
    {
        ...
        if (it != cache.end()) {
            ...
            try {
                ...
                if (settings.count("autoFetch")) {
                    hud.m_bAutoFetch = (settings.at("autoFetch") == "1");
                }
            } catch (const std::exception& e) {
    ...
    // Apply LeanWidget with enabledRows and showMaxMarkers
    {
...
void SettingsManager::saveSettings(const HudManager& hudManager, const char* savePath) {
    ...
    // Save update channel (stable or prerelease)
    ...
    file << "[Advanced]\n";
    file << "developerMode=" << (m_developerMode ? 1 : 0) << "\n";
    ...
    // Only save layouts that actually exist (default: variants 1 and 2)
    {
        ...
        for (int variant = 1; variant <= 10; ++variant) {
            ...
            file << "triggerFillMode=" << layout.triggerFillMode << "\n";
            file << "\n";
        }
    }
