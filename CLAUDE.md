...
- **Add a per-rider map to `PluginData`** (keyed by raceNum): erase it in `removeRaceEntry()` **and** reset it in `clear()`. Miss `removeRaceEntry` and a *reused* race number inherits the departed rider's stale standings/gap/position state. (Several maps were found missing when this rule was written — grep `removeRaceEntry` for the current set.)
- **Add any input to MapHud's `renderTrack()` output** (new setting, color, transform): add it to `TrackRibbonKey` (`map_hud.h`) or the ribbon quad cache serves stale geometry. `TrackRibbonKey` guards the **screen-quad** cache — it hits in the default view (rider-only rebuilds) but by design is a pass-through in rotate-to-player/zoom (the view transform changes the key every frame there). Underneath it, `renderTrack` tessellates the ribbon once in **view-independent world space** and caches that (`WorldRibbonPoint` / `WorldRibbonKey`), then transforms the cached points to screen — this is what keeps rotate/zoom cheap (they'd otherwise re-tessellate the whole centerline every frame). The two keys guard **different** things: `TrackRibbonKey` covers everything that changes the emitted *screen* quads (view transform, clip, colors, width); `WorldRibbonKey` covers only what changes the emitted *world* points (track data via `m_worldRibbonValid` in `updateTrackData`, the resolved `lodSpacing` — into which the detail **scale 20-200%** and INI-only `detailBaseline` are folded — plus `adaptiveDetail` (drives the curve-step floor) and zoom-straight-subdivision). If you make the world geometry depend on a NEW input (e.g. a second LOD knob), add it to `WorldRibbonKey` or rotate/zoom serve stale ribbon geometry. The world builder also **merges segments shorter than the sample spacing** (so segment-dense tracks don't floor at ~2 points per segment, which used to make low detail a no-op) and **dedupes the duplicated joint sample** (one degenerate quad per segment boundary per pass, pure per-quad overhead) — don't "simplify" either away. `map_render_test.cpp` asserts the world cache is transparent (identical quads across detail + rotate/zoom round-trips), that the detail dial has real range, that adaptive mode normalizes quad count across track lengths, and that legacy `detail=AUTO|HIGH|LOW` INIs migrate.
