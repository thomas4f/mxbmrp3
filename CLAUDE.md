...
- **Theme sprite order**: `discoverThemes()` assigns indices in `spriteFiles` order and `setupDefaultResources()` pushes in it; diverge and a theme draws another's sprites. **Convention.**
- **A user-selectable asset is stored by NAME, not by discovery index** (themes, icon overrides, gamepad packs): an index reassigns everyone's choice when a pack is added or renamed. An unknown name degrades to the shipped default *without* rewriting the stored value, so restoring the folder restores the choice. **Pinned** by `asset_pack_test.cpp` (both pack types).
...
| `mxbmrp3_data/gamepads/`, `pitboards/` | Asset packs: `<name>/` = art + `<name>.ini` placing content on it (a pad's 17 buttons, a board's rows). Same nested shape as `themes/`, same sync code |
