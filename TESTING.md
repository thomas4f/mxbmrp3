...
`check_lazy_module_imports.sh` (the linked artifact carries no import-table
entry for `opengl32`/`d3d11`/`d3dcompiler`/`dcomp`: all four are resolved at
runtime so a machine without them falls back rather than failing to load the
plugin, and the whole GPU -> software -> engine fallback chain depends on there
being nothing for the loader to fail on. Runs `objdump -p` over the cross-built
DLL, which is a property no source lint can see; the usual way one appears is
including the header and calling a function directly instead of through the
resolved pointer. `opengl32` is the dangerous one - it exists on every Windows
machine, so an accidental import would never fail loudly, just quietly make the
plugin a GL client on every launch),
...
`.editorconfig`), `check_pages_render.sh` (every tracked `.md` through
**kramdown**, the renderer GitHub PAGES uses - not GitHub's own cmark-gfm, and
where the two disagree the site's page silently comes out wrong: a table as a
paragraph of pipes, a row swallowed by an unclosed `<tag>`, a bracketed literal
read as a dead reference link. All three shipped in v1.29.3's README, which
rendered correctly on github.com the whole time. SKIPs without the gem),
`check_session_hook.sh` (the SessionStart hook's own
...
header (six sites across four references had, when the check first ran).
...
- `test_analytics_redact.cpp` - the install id is stripped from a request body before it is logged. It matches on the id's VALUE, not on a key name, so it covers every field carrying it (`props.install_id`, GoatCounter's `hit.session`) including ones not written yet; the cases pin that all occurrences go, that the rest of the body survives (the log line exists to diagnose ingest rejections), and that an empty or too-short id leaves the body alone instead of shredding it
...
- `test_update_version_match.cpp` - `UpdateChecker::isSameRelease`, the "is the running DLL the update we just installed?" test. The sentinel holds the release tag (`vX.Y.Z`, so build 0) while `PLUGIN_VERSION` carries a nonzero `VER_BUILD`, so the old all-four-components equality could never hold: the post-update donation nudge and the Fresh Coat achievement it feeds were unreachable for every user from the day they shipped
...
- `test_render_batch.cpp` - the shared batcher (`core/render_batch.h`), extracted from the D3D11 backend so the GL backend consumes the same batches. This file IS the point of that extraction: the batching was untested on every backend, because `hud_gpu_renderer.h` reasonably said it "needs a real device AND a display, and headless CI has neither". True of device creation and presentation; never true of the geometry, and the geometry is most of the function. With handles reduced to opaque pointers it all runs headlessly: NDC mapping (both APIs put clip-space y=+1 at the TOP, which is why the mapping is shared verbatim rather than branched), corner order and the two-triangle split, colour passing through bit-identical (a swizzle here would tint the whole HUD), run coalescing by texture and where it must split, `firstIcon`'s texture-vs-icon boundary, a decode miss skipping only its own quad, out-of-range sprite and font indices, strings after quads (z-order, or every label falls behind its panel), justify offsets, a space advancing without emitting, and glyph UVs normalised by the atlas. What these tests CANNOT do is prove the extraction itself was faithful - the pre-extraction code needed a device to run - so that rests on the move being mechanical (only the signature, handle types and cache lookups changed) plus in-game re-validation of the companion window with `hwAccel` on
- `test_gl_state_fingerprint.cpp` - the comparison half of the Phase 0 GL probe (`core/gl_state_fingerprint.h`). The probe's safety claim is "we drew in the game's context and left it exactly as we found it", and `diff()` returning 0 is the whole evidence for it - so a `diff()` that misses a change would print a clean bill of health for a renderer leaking a bound program into the game's next draw. Every case plants a difference and demands it be seen: a leaked `CURRENT_PROGRAM`, one component of a multi-value token (named with its index), an unbalanced push/pop showing only as a stack-depth change, and two samples of differing SIZE, which must be a difference rather than a shorter compare. Plus the version/profile gate, since querying an unsupported token raises `GL_INVALID_ENUM` in the GAME's context
- `test_hud_sw_renderer.cpp` - golden-frame sampling of the companion window's software renderer (`core/hud_sw_renderer.cpp` compiled natively): quad fill, per-quad alpha, the texel×color modulate (white-icon tinting), `.fnt` text against a real shipped font, and the scale-viewport mapping. Also the FIELD BUG in minified text: the shipped 135px cell against ~20px HUD text is a ~7x minification, and one bilinear tap reads 4 texels of a ~49-texel footprint, dropping exactly the partial-coverage texels at a stroke's edge. The instrument is the coverage histogram's SHAPE rather than a pixel count, because undersampling collapses pixels toward all-or-nothing - measured at 394 lit pixels/mean 201 without the mip chain against 805/93 with it, so the thresholds sit in the gap and removing the mip path fails the case outright
- `test_render_asset_decode_mips.cpp` - the glyph-atlas mip chain (`hudassets::buildFntMips`), which all three renderers sample text from, so a fault here is a fault in every backend at once. Pins the box filter on coverage (a uniform field survives the whole chain; a half/half split averages to mid-grey), that an ODD axis still shrinks so the chain cannot stall, and that it terminates at 1x1 - that last one is not cosmetic: GL 1.1 has no `GL_TEXTURE_MAX_LEVEL`, so a chain stopping early makes the texture INCOMPLETE and it samples as white. A shipped font is run through the real decode too, so the filter is proven on the actual 2048² atlas and not only on synthetic input. Also `buildTexMips`, the ICON chain, whose extra hazard is the premultiply: a .tga's transparent surround still carries colour bytes (usually black), so a straight RGBA average rings every icon with a halo that deepens at each level - the case pins one opaque white texel beside three transparent black ones, where naive averaging gives a grey rim and weighting by alpha gives back white at quarter coverage. Plus the divide-by-zero a premultiply invites (a wholly transparent footprint), that a constant alpha cancels out exactly, and that `decodeTga` builds NO chain on its own - nine-slice panel art must not get one, and that only holds while the caller decides
...
- `test_roost_detect.cpp` - `core/roost_detect.h`, the pairwise proximity core behind Roost (Side by Side no longer has a row, but is still classified - it is how the detector says "alongside, not behind"). World distance decides who is near (exact, and lap-number-free); the centreline decides only ahead-or-alongside, and the two bands are built not to overlap so a tick is never credited twice. The cases that earn their place are the ones a naive "are they near me?" test gets wrong: two riders nose to tail ACROSS the start/finish line are 4m apart and not most of a lap, a rider behind you is not your roost however close, a rider near in the world but half a lap away on a hairpin is nobody's contact, and the CLOSEST rider decides - at most one contact is credited per tick, or a mid-field scrap would be worth more per second than a fight for the lead
...
- `test_completion_floor.cpp` - `core/completion_floor.h`, the Completionist row's number. It is a FLOOR over the rows a player can see, not an average and not a percentage, so the cases that earn their place are the ones the obvious loop gets wrong: the hidden group must not hold it down (with those rows counted, Bronze required a self-compiled build, a hand-edited stats file, developer mode, a crash to desktop, two rows of pure luck and a year's wait, so nobody but the author could ever hold it), a FINISHED one-shot must read as full rather than pegging everyone at Bronze forever on its earned tier of 1, one visible row short must take the whole thing down to that row's tier, and an empty visible set must read 0 rather than the vacuous "everything done" that an unguarded all-of loop returns. Two cases carry a REQUIRE on the catalogue actually containing hidden rows / visible one-shots, so they cannot pass by proving nothing
- `test_finish_margin.cpp` - `core/finish_margin.h`, the winner-to-runner-up margin behind Photo Finish. It differences the two riders' lap logs rather than reading `StandingsData::gap`, which is cached across the flag (the API zeroes gaps as the leader crosses and PluginData substitutes the last live value so the standings do not flicker) and is a sub-lap remainder besides - between them a false negative that held Photo Finish to 1 unlock in 1728 installs and a false positive that scored a rider a full lap down
...
| `reset_tab_test.cpp` | **Per-tab "Reset `<tab>`" completeness**: for every settings tab, clicks every control the tab emits, presses that tab's Reset, and requires the saved INI back at factory. Neither side is a list in the test - the tabs enumerate themselves and their own click regions say what they can change - so a control added to a tab is covered the day it lands. The one maintained list is what a per-tab reset deliberately leaves alone (visibility, master switches, the analytics opt-out). Caught four settings the General tab never restored (Grid Snap, Screen Clamp, Direct GL, Analytics), the Crash widget and menu-only cursor the Widgets tab skipped, and the Spotter widget's opacity/scale |
...
| `hotkey_capture_test.cpp` | **The hotkey-capture lockout.** Clicking a bind row arms a capture, and an armed capture deliberately swallows the whole keyboard (`HotkeyManager::update` routes into `updateCapture` and `processHotkeys` returns early) so that any key, bound or not, can be chosen. Its only cancel was the ESC inside `SettingsHud::handleInput`, which needs the panel open - so closing the menu on an armed capture killed every hotkey in the plugin, including TOGGLE_SETTINGS, with no way back but a restart. Pins both halves of the fix separately: `hide()` disarms it immediately, and the draw path treats "capturing while the menu is shut" as impossible so no future close route can strand one |
| `achievements_test.cpp` | **Achievements** end to end: a race win awards its tiers at the record point with one toast each, persists in the stats JSON by id and does not re-toast on the next start; a pre-achievements stats file is granted silently on load with ONE summary toast that counts achievements (not tiers), its stored PBs backfilling `pbCount`; `[Achievements] visible=0` still earns and stores but queues nothing; the RELOAD_CONFIG feed (Tinkerer) fires from a fresh install and the widget shows the toast on the next draw; a backflip and then a wheelie through the real FMX detector reach the lifetime totals (airtime, wheelie hold and distance, best chain, trick kinds) and the `fmx` block only when each chain completes; a wet twenty-rider race feeds Mudder, Crowd Surfer, Marathon (a max), Globetrotter and Collector; the totals behind the rows agree between the clean-cache bumps and a recompute; a toast waits while the hide-all-HUDs hotkey hides the widget; the tab's pager buttons page through the stand-in mouse and an end that cannot be passed has no region; a click on a toast opens the menu on the Achievements tab and ends the card; a frontflip is its own row and a third trick kind; Clean Sheet is the best penalty-free run, Leap Forward a PB beaten by a full second, Iron Butt the longest session read back from the file |
| `hidden_input_test.cpp` | A HUD the frame is not drawing **takes no input**: the hide-all hotkey and the Widgets toggle hide at draw time and leave the visibility flags alone, which is what the input pass read, so a hidden HUD could still be right-dragged and its click targets still fired whenever the cursor was up. `HudManager::isHeldBack` is now the one answer both passes read (`BaseHud::isHeldBack` for the HUDs that hit-test their own clicks). Drives the stand-in mouse (`MXBMRP3_Test_InjectMouse`) through the real frame path: a hidden HUD stays put under a drag that moves it once shown; the widgets toggle holds back a widget and not a full HUD. The Direct GL prompt's half of the same bug is in `gl_render_test.cpp` |
| `exploration_test.cpp` | The **exploration and hidden achievements** (`core/exploration_stats.h`) end to end: a startup notices a user pack and a styled `custom.css` and grants them silently with one summary card, a new day counts, the small hours are Night Owl and the first run's anniversary is Anniversary on the injected clock; a settings or stats file the plugin wrote loads clean and the same file edited by hand earns Under the Hood / Backup Plan by its own fingerprint; Photo Finish, Wire to Wire, Lapped the Field and Charger from the finishing classification, Rage Quit from leaving a race, Sandbagger from a last-lap PB; Palindrome, Deja Vu and Metronome from lap times; Baker's Dozen and the ninety-nine crash nod on the crash edge; the once-a-second tick's time sums, Frame Perfect, On Air and Steady Hands; Ungrateful with the toasts off; a five-backflip chain crashed out is Case of the Mondays |
...
| `prestige_test.cpp` | **the prestige trade**: refused until the Platinum Sweep is earned (the act re-checks, so the button being drawn is not the rule), with **developer mode as a second key** that opens both the act and the badge widget; it zeroes every achievement tier and the lifetime counters they read, **keeps the personal bests** with an honest `pbCount` of 0 (the floor in the load path used to hand Personal Best straight back), and the level survives the next load. Also the badge widget drawing nothing until there is a level, and a per-track / per-bike maximum row naming the track or bike carrying it - by display name, falling back to the track id |
| `preview_test.cpp` | **the positioning preview**: a HUD that draws nothing until something happens fills itself with placeholder content while its own settings tab is open, so it can be dragged into place - and a **disabled** HUD does not, since opening a tab is how you reach its switch rather than a request to see it. Driven through the achievement toast (its idle state is real and reachable); the header records why the radar and the notices panel are the wrong instruments for it |
...
| `fuel_stats_test.cpp` | **the fuel achievements**: litres burnt accumulate from the tank level FALLING (a refuel re-references rather than counting, and a new bike's full tank is not a burn against the last one's empty one), the burn rides the odometer's ~100m flush so an evaluation never lands on a 100Hz path, Long Walk Home is the EDGE of the tank going dry rather than every tick spent there, and Running on Fumes reads the level at the flag. The must-catch case is an UNKNOWN tank (`maxFuel` 0, what the API reports with fuel consumption off): a level of 0.0 beside it has to read as "cannot be known", never as a dry tank, or every player without consumption earns both rows on their first lap. The accounting lives in StatsManager and not FuelWidget precisely so it does not depend on a HUD being switched on |
| `proximity_test.cpp` | **Roost end to end**: the seconds come off the real `RaceTrackPosition` batch through PluginData's geometry pass into StatsManager's accumulation - not off a HUD or the spotter, both of which a player can switch off. Pins the gating (a race actually in progress, so neither practice nor the pre-start grid, where the whole field sits wheel to wheel doing nothing, counts), that a crash ENDS the interval rather than pausing it (a paused clock would credit the time on the floor when racing resumed), and that the ~1s flush hands the total over without dropping the remainder. The geometry itself is `test_roost_detect.cpp`'s |
| `fmx_test.cpp` | **FMX trick detection + scoring** through the real RunTelemetry path under the injectable FMX clock (`Fmx::clockNow()` / `MXBMRP3_Test_FmxSetNowUs`, 10ms sim steps): a sub-debounce hop banks nothing (airborne debounce); a sustained airborne full-pitch rotation classifies as **BACKFLIP**, survives the 0.75s landing grace, and banks a non-zero score into the session when the 2s chain window expires; a crash during grace fails the trick without touching the session score (state via `MXBMRP3_Test_FmxState`). Plus the **Air group's split rule**: a rhythm section credits every hop (not just the last), the three SUMS are banked at touchdown so a jump you case out of still pays them, and the three MAXIMA need the settle window survived - including Hang Time, which reads the flight rather than the trick, so a hop nobody tricked on sets it |
...
| `gl_render_test.cpp` | **The in-context GL backend's actual rendered OUTPUT** (`core/hud_gl_renderer.h`) - coverage no GPU backend in this repo previously had. Under Xvfb the harness makes a real GL context on the thread that calls `draw()`, so the backend renders a synthetic frame and the test reads pixels back: a quad lands where the coordinate mapping says AND does not bleed past its edge, colour is not swizzled (a red/blue swap would tint the whole HUD), the later primitive wins the z-order (submission order IS z-order; if run coalescing ever reordered, panels would swallow their own labels), the shader-free `GL_ALPHA`/`GL_MODULATE` text path puts ink on screen, and the game's context comes back byte-identical afterwards (the probe's own 51-value fingerprint, applied after a full render rather than one test quad). This is Mesa/llvmpipe under Wine, not a vendor driver, so it is weak evidence about any particular player's GPU and strong evidence about what is the same everywhere. It earned itself immediately: the text case caught an incorrect vertical flip on texture upload that the quad cases structurally could not see, because they draw through the 1x1 white texture, which is its own mirror image. Without a display the cases say so rather than passing silently. Two later cases sit at a different altitude and are the ones worth reading: a shader program the game left bound is the ONE failure this backend cannot feel (everything else raises a GL error, which `render()` turns into a clean engine fallback; a bound program raises nothing and silently replaces fixed-function processing), so a program painting pure BLUE is planted and a RED quad asked for - a regression arrives visibly wrong rather than subtly. And the frame's asset tables are asserted to carry RENDER names rather than the game's full asset paths: `renderInContextGl` handed over the path table, every lookup missed, the batcher skipped the primitive and nothing raised an error, so the HUD lost all text and every textured sprite while untextured fills kept drawing perfectly. Every pixel case above passes with that bug present, because the probe builds its own frame from a hand-written basename and never routes through the line that was wrong - which is the transferable lesson: a test is evidence only for the code it actually executes. Two further cases pin the CONFIRMATION PROMPT (`hud/gl_confirm_hud.h`), both must-catch: silence REVERTS the `glInGame` setting when the countdown expires - the tester whose glyphs came out as solid blocks could not find the row to turn it off again, and hand-editing the INI was the only way back; the prompt goes THROUGH Direct GL rather than round it, so it is its own test (an engine-drawn prompt renders perfectly however badly GL is failing, which makes clicking Keep prove nothing); the escape needs no readable text, pinned as the panel emitting quads of its own, since the observed failure took glyphs while quads kept drawing; it reaches the frame on the pluginThread path, asserted as the worker's frame GROWING by the panel's own quad count when armed; and NO FRAMES MEANS NO TIME - real seconds pass with no Draw and the countdown must not move (menus issue no Draw, so it cannot expire behind a pause screen), then frames resume and it must advance at the RIGHT RATE rather than a fraction of it. That second half is the interesting one: the first version gated on glDrewLastFrame(), which reads false on every frame renderInContextGl is called with nothing to draw - most of them, on the pluginThread path - and each false sample discarded its interval, so ten seconds became minutes in game while measuring perfectly in a harness that drew every frame - the first version routed the prompt to the engine from the sync branch of handleDraw only, so with pluginThread on it was built every frame and drawn by nothing, which is how it was found in-game rather than here. The countdown is advanced by a hook in DRAWN milliseconds: a test that had to render for twenty real seconds is a test nobody runs |
| `gl_probe_test.cpp` | **The Phase 0 GL feasibility probe is inert unless it can be sure** (`core/gl_probe.*`, `[Advanced] glProbe`). Its headline question - is a GL context current on the game's Draw thread - needs the game; what is pinned here is everything that would waste that one launch or harm a player who left the key on. At every mode, GL present or absent, `lastGameQuads()`/`lastGameStrings()` are unchanged: the probe draws into the GL context, never into the frame the engine is handed. With no context, mode 2 must still not draw (the branch a non-GL game takes), and `stateDiffs` must read -1 for "not measured" rather than the 0 that would be indistinguishable from "measured clean". The third case is decided by what Wine can do, like `gl_render_test.cpp` below: with a display the hook brings up a REAL GL context on the harness's Draw thread and the probe's own machinery runs against a real driver - conservative save/restore, the fingerprint diff, the pixel readback - with `stateDiffs == 0` and zero GL errors as the assertion the case exists for; headless there is no GL and the test says so out loud rather than passing silently. Test header has the Xvfb recipe |
