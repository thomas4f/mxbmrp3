...
| RunStart / RunStop | RunStart: sim resume (⛔). RunStop: leave-track (pits) → settings flush + stats save | 🟠 / ⛔ | RunStop the pit-flush path is prod-only (not driven headless); the equivalent flush is asserted via the settings-defer hooks + RunDeinit. RunStart: no headless effect |
...
| RunTelemetry | player speed/rpm/gear/lean/fuel → widgets/helmet + stats (distance/top speed) | 🟡 | perf; stats_test (top speed persisted + the `finiteOrZero` +Inf write guard). Widget/helmet rendering ⛔ (not in /api/state); distance/odometer accumulation still a **gap** (needs wall-clock-spaced ticks) |
...
| Stats: odometer / distance / top speed / personal bests | derived | **B** (stats JSON) | ✅ | stats_test (personal-best lap persist + faster-replaces-only; top speed + `finiteOrZero` guard). **Deferred**: the PB is NOT on disk mid-ride and is flushed on RunDeinit (the "no save while on track" rule). Odometer/distance accumulation still a gap (wall-clock ticks) |
...
| FMX trick detection / scoring | derived | state (not JSON) | ⚪ | **untested — whole subsystem.** FmxManager is fed by RunTelemetry and its score/chain is in-game-only (not in `/api/state`), so it needs a `MXBMRP3_Test_*` hook exposing `getScore()`/`getChainTricks()` + telemetry orientation sequences (e.g. a backflip) to drive it |
| Records fetch (MXB-Ranked / CBR) | user / auto | D (net) + C (parse) | ⚠️ | parse-with-canned-response testable; live fetch manual |
| Rumble effects | derived | D (feel) + C (math) | ⚠️ | effect math via hook; vibration manual |
...
7. ~~**Personal bests / stats file, ideal lap, posDeltaSplit**~~ — ✅ done: stats_test (PB persist + faster-replaces-only, top speed + `finiteOrZero` guard), ideal_lap_test (`idealLapMs`), posdelta_split_test (positions gained since the last split). Remaining: **distance/odometer** accumulation (wall-clock-spaced telemetry). (RaceHoleshot / RunLap are intentionally unexported/stub — not gaps.)
...
Unlike analytics/Discord, FmxManager *is* compiled into the headless build (game-gated
on, not excluded), and its score/chain are already public (`getScore()`,
`getChainTricks()`), so a hook is trivial. The blocker is timing: `updateFromTelemetry`
computes `dt` from `steady_clock`, so feeding frames back-to-back gives `dt≈0` and the
airborne/rotation accumulators never advance — the same wall-clock problem the director
had. The seam it needs: an injectable `dt` (or clock) under `MXBMRP3_TEST_BUILD` — mirror
`DirectorManager::testSetNowMs` — plus a `MXBMRP3_Test_Fmx*` hook exposing score/chain,
then drive a crafted orientation sequence (e.g. a full-pitch rotation while airborne past
the 0.5 s debounce = a backflip) and assert a non-zero banked score + the detected
`TrickType`. Sized like the director-broadcast harness; deferred here so it lands as a
real detection test, not a trivially-true one.
