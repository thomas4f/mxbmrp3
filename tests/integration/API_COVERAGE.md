...
| RaceHoleshot | holeshot event | ✅ | exported to log + record only - the game never fires it (measured; `handlers/opening_line.h`), so the Holeshot-ish row reads the first RaceSplit instead; recorder_test pins the tape round-trip |
