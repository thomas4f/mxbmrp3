...
```bash
tools/hud_window/companion_demo.sh out.png       # runs under Wine + Xvfb
tools/hud_window/companion_demo.sh out.png 12 tab Map   # a specific settings tab
tools/hud_window/companion_demo.sh out.png 25 preview   # the theme-preview scene
SHOT_RES=2560x1440 tools/hud_window/companion_demo.sh out.png  # custom resolution
```

The scene modes are named in `companion_demo.cpp`; `preview` is the one worth
knowing about from outside, because it is a fixed reference rather than a
showcase. It stages five HUDs - Position, Lap, Timing, Standings, Records - with
a six-rider race grid and the server records loaded, so a renderer can be lined up
against the real one at a scene that does not move. It was built for a theme-preview
page that has since been deleted; the scene survives it because a fixed reference
scene is useful on its own. Its own README gives the settings the comparison
needs seeded:

```bash
EXTRA_INI=$'panelTheme=carbon-dark\n\n[RecordsHud]\nvisible=1' \
    tools/hud_window/companion_demo.sh preview.png 25 preview
```
