...
via `./tools/install_deps.sh codeql`). It is the only gate that is *also* a CI
job elsewhere: `codeql.yml` can run only on the **public mirror** (code scanning
needs Advanced Security), and the mirror only receives code at release time — so
without a local run, the first scan of a change is its release. That is exactly
how v1.28.0 shipped and then collected three alerts. Budget ~10-15 min: it
